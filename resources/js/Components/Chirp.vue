...
<!-- TODO: understand why page.props.auth.user.id is used -->
                <!-- <Dropdown v-if="chirp.user.id === $page.props.auth.user.id"> -->
                <Dropdown v-if="chirp.user.id">
                    <template #trigger>
                        <button>
                    ...
                    <template #content>
                        <button class="block w-full px-4 py-2 text-sm leading-5 text-left text-gray-700 transition duration-150 ease-in-out hover:bg-gray-100 focus:bg-gray-100" @click="editing = true">
                            Edit
                        </button>
            ...
            <form v-if="editing" @submit.prevent="form.put(route('chirps.update', chirp.id), { onSuccess: () => editing = false })">
                <textarea v-model="form.message" class="w-full mt-4 text-gray-900 border-gray-300 rounded-md shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"></textarea>
